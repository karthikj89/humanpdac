...
if __name__=="__main__":
    ...
    if args.command == 'prepare':
        ...
        if args.counts.endswith('.h5ad'):
            subprocess.call('cp %s %s' % (args.counts, cnmf_obj.paths['tpm']), shell=True)
            tpm = sc.read(args.counts)
            input_counts = sc.AnnData(X=tpm.layers['counts'])
            input_counts.obs_names = tpm.obs_names
            input_counts.var_names = tpm.var_names
        else:
        ...
        if args.genes_file is not None:
            ...
            highvargenes = tpm.var_names #[tpm.var['highly_variable']]

        print(input_counts[:, highvargenes].shape)
